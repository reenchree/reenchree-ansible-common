...
| `github_release_exporter` | **Internal** parameterized role backing both exporters: install a Prometheus exporter from a GitHub release tarball + hardened systemd unit. The stderr-capturing version check lives here (once) | required from caller: `_org` `_repo` `_version` `_binary` `_unit_name` `_description` `_listen_address`; `_after`/`_extra_args`/`_arch` optional. Not called directly |
...
| `sea-misc-ansible` | `nut_server` (the only consumer of that role), `ops` |
...
roles/
  ...
  github_release_exporter/ defaults handlers tasks    # internal, backs both exporters
